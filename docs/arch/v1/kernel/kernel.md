...
The kernel receives `job_envelope_v1` from the API Gateway — already trusted, already dispatched to the right shard. The kernel executes. See `docs/arch/v1/api-gateway/api-gateway.md` for gateway design and job envelope schema.
...
| `append` | Append a new record to append-only history without overwriting prior state. |
...
Any running job may call another skill by emitting another command:

```
octos <root_skill>.<nested_skill> [args...]
```

If a skill must be passed as input to another skill, encode it as:

```
--skill.<param>=skill:<skill_id>
```

A nested skill call is valid only when the nested skill is provisioned inside the root skill's contract.

The kernel does not trust caller-supplied lineage metadata. Parent linkage, call depth, and intent scope are assembled and enforced by gateway/kernel execution context.

Protocol contract: `docs/arch/v1/skill/skill-composition-protocol.md`.
...
## Versioned Contract Dispatch

The kernel executes skill contracts by version. It never mutates behavior for an existing version.

Dispatch rules:

1. Read `protocol_version`, `skill_schema_version`, and `min_kernel_version` from the skill contract in Redis.
2. Reject dispatch if kernel version is below `min_kernel_version`.
3. Select the versioned execution handler by `(protocol_version, skill_schema_version)`.
4. Reject unknown or unsupported versions.
5. Add new behavior by introducing new versions, never by modifying existing version handlers.

This keeps the kernel open for extension and closed for modification at the contract boundary.
...
The primary scheduled skill is `reasoning` — fires when the user is offline, reads unprocessed session history + VAD, and produces observational nodes and edges. Exact schedule TBD. See `docs/arch/v1/skill/skill-reasoning.md`.
...
- `docs/arch/v1/skill/skill-lifecycle.md` — full skill lifecycle: observation → intent namespace → validation → skill building → provisioned
