...
A skill constructor is the context node set used at skill start.
...
```bash
skyra planner --ctor.user=node:user.mike --ctor.goal=node:goal.fitness --query="best next action"
```
...
- `node:<node_id>` is the canonical constructor reference format.
