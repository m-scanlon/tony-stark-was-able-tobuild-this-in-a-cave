...
| G23 | Internal CLI vs external MCP tool transport contract not locked | P1 | Skyra needs a deterministic transport split: internal capabilities execute through a strict CLI adapter (`argv`, no shell-string execution) while external capabilities execute through MCP. Without a locked contract, malformed-call handling, policy enforcement, and audit trails diverge across tool paths. Missing pieces: capability metadata (`provider=internal|external`), resolver behavior, shared invocation envelope, and normalized result/error schema across CLI and MCP executions. | Tooling + Orchestrator + Agent Service |
...
| G25 | Skyra binary invocation contract for internal tools is not defined | P1 | A concrete CLI execution contract is missing for internal tool dispatch (for example `skyra <shard> <tool> <args...>`). Without a locked binary interface, internal tool execution behavior can drift by shard and runtime. Missing pieces: command grammar, argument typing/encoding, versioning, exit code semantics, stdout/stderr result envelope mapping, and policy guardrails (no shell-string mode on default path). | Tooling + Executor + Platform |
