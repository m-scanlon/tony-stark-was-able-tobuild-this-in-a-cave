...
| G4 | ~~Reconciliation UX policy is not fully specified~~ **CLOSED for v1** | P1 | **v1 decision**: Provisional responses cut from v1. Pi emits non-semantic ACKs only (earcon/LED/short wait phrase). Pi renders only Mac-authored messages (`UPDATE`, `PLAN_PROGRESS`, `CLARIFY`, `PLAN_APPROVAL_REQUIRED`, `FINAL`, `ERROR`). No split-brain risk in v1. Provisional response path (front-door model speaks before Mac responds, then reconciles on `FINAL`) deferred to v2 — see note in `docs/arch/v1/scyra.md` section 7.1. | Voice Node + Control Plane |
...
| G12 | ~~Agent state (state.json) four-section schema not locked~~ **CLOSED** | P0 | Schema locked: `metadata/knowledge/artifact/boundary`. Boundary carries `allowed_tool_categories`, `denied_tool_patterns`, `restrictions[]` (no enforcement field — all locked tool attempts prompt the user). Enforced in code via two layers: hydration (lock status attached to tools before LLM sees them) and BoundaryValidator (permission prompt at runtime). See `skyra/internal/project/README.md`. | Agent Service |
| G13 | ~~Tool system two-layer contract not finalized~~ **CLOSED** | P1 | Global vs local tools, `requires_approval` behavior, and local tool `categories[]` field for boundary enforcement are now fully specified. See `skyra/internal/project/README.md`. | Agent Service + Domain Expert |
| G14 | Voice response channel (`/v1/voice`) not implemented | P0 | `/v1/voice` returns a literal string, not a `voice_result_v1` protocol message. No Mac response can reach the user over the voice path. Blocks all voice UX end-to-end. Needs proper streaming response channel (WebSocket or gRPC) emitting `message_type` in `{UPDATE, PLAN_PROGRESS, CLARIFY, PLAN_APPROVAL_REQUIRED, FINAL, ERROR}`. | Control Plane |
| G15 | Plan approval response channel undefined | P1 | Pi has no mechanism to route a user's spoken approval/rejection (`APPROVE \| REVISE \| CANCEL`) back to a `PLAN_APPROVAL_REQUIRED` job. `session_state.pending_job_id` + `waiting_for` contract is defined in the event schema but not implemented anywhere. A user saying "yes" after a plan approval prompt is indistinguishable from a new request. | Voice Node + Control Plane |
...
| G17 | Agent routing staleness — Pi `active_agent` may be stale | P1 | Pi sends `active_agent` from local cache in `context_window`. Mac has no mechanism to verify or correct it. If the user switches agents, Mac routes to the wrong context until Context Injector sync is implemented. Preferred resolution (Context Injector push to Pi listener cache) not implemented. | Context Injector + Voice Node |
| G18 | Cross-agent write protocol undefined | P1 | `skyra.user` is always injected (read direction solved). The write direction is not designed: if Skyra learns something about the user during a domain agent session, there is no defined protocol for propagating that to `skyra.user` without breaking single-ownership semantics. v1 behavior: manual commit only, Skyra flags insights at session end. v2 direction: LangMem background manager extracts candidates from session history → user confirms at end-of-session → committed via normal commit flow. Nothing auto-commits. See `docs/arch/v1/agents/user.md`. | Agent Service + Orchestrator |
| G19 | Multi-domain session invocation undefined | P2 | The current model routes each session to one active domain agent. Requests that naturally span multiple domains (e.g. a conversation that involves both gym tracking and developer notes) have no defined handling. Three options under consideration: (A) primary domain + post-session LangMem extraction to surface cross-domain candidates, (B) multi-agent context injection in one session, (C) split into separate jobs per domain. None decided. Depends on G20 being resolved first. See `docs/arch/v1/agents/README.md`. | Orchestrator + Agent Service |
| G20 | Session model is undefined | P1 | "Session" is used throughout the architecture but never formally defined. Current usage implies session = one LLM context = one job. But the Pi's `session_state` implies sessions can span multiple turns (pending jobs, waiting states). LangMem extraction (G18) requires knowing what a "session" is before it can process history. Multi-domain invocation (G19) depends on the session boundary. Open questions: is a session a single turn? A conversation block until the user goes idle? A domain work block? How does a session end? Who closes it? | Orchestrator + Voice Node |
...
4. **Define user-facing consistency behavior (`P1`)**
   - ~~Formalize provisional phrasing policy on Pi.~~ **v1 decision**: provisional responses cut. Pi speaks non-semantic ACKs only. See G4.
   - Implement voice response channel (`voice_result_v1` protocol over WebSocket/gRPC). See G14.
   - Define and implement plan approval response channel so Pi can route user decisions back to pending jobs. See G15.
