...
`Jarvis` is the canonical name for the user-facing orchestrator node.

References to the older `Life` concept should collapse into `Jarvis`.

Functional subtitle:

- `Jarvis` = user-facing orchestrator node

Jarvis is not outside the node model.

It is a node whose role is to track what matters, shape user-facing meaning, and coordinate user-facing work across other nodes.
...
- route user-facing work to the right participating nodes when necessary

Jarvis is not the structural node.

It is the meaning-and-attention counterpart to Stark.

## Same Model As Other Nodes

Jarvis uses the same broad data model as other nodes.
...
- Jarvis is an orchestrator node
...
- typed outputs from other nodes that need user-facing synthesis or routing
...
- Jarvis is a node, not a hidden persona layer
- Jarvis is an orchestrator node, not a base worker node
- Jarvis shapes meaning and attention; it does not own structure
- Jarvis uses the same general runtime model as other nodes
- Jarvis is the canonical user-facing orchestrator node

## Relationship To Stark

Jarvis and Stark are paired major node roles.
...
It tends continuity, attention, and what matters from the user-facing side of the system while coordinating other nodes when needed.
