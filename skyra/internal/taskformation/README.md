...
- Task artifacts (WorkPlan/TaskSheet) are persisted in the object store under `.skyra/projects/{project_id}/jobs/{job_id}/tasks/{task_id}/`.
- Task formation operates within a two-layer tool system: global tools (always present) and local tools (retrieved per request via vector search from the project's tool registry).
