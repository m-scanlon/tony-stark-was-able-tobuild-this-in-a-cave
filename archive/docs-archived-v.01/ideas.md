...
- Custom hostname (`skyra-actor-1`, `skyra-voice`, etc.)
...
- Do all Shard actors run the same base image, or is there a Voice Shard variant and a desktop variant?
...
The structure isn't a list of rules or two flat files. It's a graph where every actor is a trait or value, and every actor carries two weights:

- **Decision weight** — how strongly this actor influences what the system does
- **Presentation weight** — how strongly this actor influences how the system says it

Soul-heavy actors (e.g. "never fabricate", "honesty over confidence") carry high decision weight, low presentation weight. Personality-heavy actors (e.g. "dry humor", "concise pacing") carry low decision weight, high presentation weight. Middle actors pull both ways — "steady progress over bursts" affects what gets recommended *and* how urgency gets framed.
...
Conflict resolution falls out naturally. If two actors pull in opposite directions on a decision, the weights determine which wins. No explicit priority rules needed — the structure handles it.
...
The values check (axis 4) is not a tradeoff — it's a veto. A decision can be right on every other axis and still be wrong if it violates a high-weight soul actor.
...
**Actor weights can shift over time:**

If actors have weights, those weights can drift — not the values themselves, but how strongly they pull. That's a more precise model of how a system (or a person) actually matures. You don't stop valuing honesty. But "steady progress" might pull harder as the system deepens. Weight drift is the right mechanism for evolution, not rewriting the soul document.

**Open questions:**
- What's the right representation for the graph at training time — structured examples, annotations, something else?
- How do you detect when a actor weight has drifted enough to warrant review vs. normal variation?
...
**Storage options:**
- **ClickHouse** — columnar, fast analytical queries, handles high write volume. Best fit for querying decision patterns across large span history (e.g. "all jobs where X tool was called and result was reverted"). Runs on a single actor.
