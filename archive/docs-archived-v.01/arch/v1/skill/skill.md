...
skill {
  ...
  protocol_version:     string          // native protocol contract version (e.g. "np.v1")
  skill_schema_version: string          // skill schema version (e.g. "skill.v1")
  min_kernel_version:   string          // minimum kernel version required to execute this skill
...
**Versioned contracts are bedrock.** Protocol and schema versions are part of the skill contract. New behavior is introduced by adding a new version. Existing versions remain immutable and executable under their original contract.
...
Inside the declared aspects: unconstrained reasoning. Outside: nothing. Improvement proposals surface via `propose_commit`. See `docs/arch/v1/skill/skill-improvement.md`.
...
When a skill needs another skill as an input parameter, use:

```
--skill.<param>=skill:<skill_id>
```

`skill:<skill_id>` is the canonical reference format for a skill input value.

Full protocol (intent scope, lineage, limits, errors): `docs/arch/v1/skill/skill-composition-protocol.md`.
...
intent expressed → no skill found → intent namespace provisioned (observational)
  ...
  → user approves
  → skill actor committed (new id, model_id stamped)
  → memory namespace provisioned alongside it
  → provisioned in Redis
  → executable
```

See `docs/arch/v1/skill/skill-lifecycle.md` for the full lifecycle.
...
| `reasoning` | Background cron job. Session history + VAD → observational actors + edges. |
...
- `docs/arch/v1/skill/skill-lifecycle.md` — how skills are born, crystallize, and version
- `docs/arch/v1/skill/skill-improvement.md` — bounded self-improvement loop
- `docs/arch/v1/skill/skill-reasoning.md` — reasoning primitive
