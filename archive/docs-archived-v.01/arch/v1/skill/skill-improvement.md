...
The committed algorithm only changes through `provision_skill` of a new version (legacy term: `update_skill`) with user approval. Same trust rules as any other committed actor. Same crypto guarantees. Same append-only history.
