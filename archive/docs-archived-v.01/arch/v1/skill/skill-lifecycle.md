...
Signals accumulate in the observational layer. When a domain exists that matches the signal, a `belongs_to` edge is added. If no domain matches, the actor exists without one — domain is retrieval scaffolding, not a structural requirement.

```
signal arrives
  → observational actor created
  → does a domain exist for this?
      yes → belongs_to edge added
      no  → actor exists without a domain — valid state
```
...
When the user is offline, the Cron Service fires a background reasoning skill. Skyra reads unprocessed session history and VAD time series, married on `turn_id`, and reasons them into observational actors and edges. This is a graph mutation event — Skyra is appending to the graph.

Actors produced here have no domain yet. Domain edges are added as retrieval structure emerges from clustering.

---

## Domain Proposal

When observational actors cluster into a coherent pattern around a recognizable area of the user's life:
...
cluster crosses threshold
  ...
  → user approves → domain actor created, belongs_to edges added to cluster
  → user declines → threshold resets, actors continue accumulating
```
...
The signal is already in the graph: **the intent namespace**. When the user expresses an intent and no skill exists to handle it, an intent namespace is provisioned as an observational actor. That actor accumulates across sessions. When Skyra observes that the same intent namespace has been provisioned repeatedly and never crystallized, that is the signal.

```
intent actor: log_workout
  → appears in session 1, session 3, session 4, session 7
  → no skill actor covers this intent
...
user approves
  → skill actor promoted to committed layer
...
Day 0:
  ...
  → observational actors + edges appended to graph
      ↓
  actors cluster → domain identified → proposed → approved
  → domain actor created, belongs_to edges added
      ↓
  patterns cross threshold → skill proposed → approved
  → skill actor promoted → Redis provisioned → executable
```
...
```
intent (input)
  → skill crystallizes from recurring intent actor
...
- No domain for a actor → actor exists without one, valid state
- Domain exists, no skill covers the intent → accumulates as observational actors, intent namespace provisioned
...
| `reasoning` | Background job triggered by cron. Decomposes session history + VAD into observational actors, then writes edges to the graph. |
