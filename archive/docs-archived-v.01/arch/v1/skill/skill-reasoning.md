# Skill: Reasoning

The reasoning skill is how Skyra builds her memory. It runs as a background job when the user is offline. It takes raw session data and turns it into graph actors and edges in the observational layer.

**Triggered by**: Cron Service
**Layer**: observational — Skyra writes freely, no user gate
**Type**: system primitive skill — pre-provisioned in Redis at boot

---

## The Job

Two sequential tasks. Task 1 must complete before Task 2 runs.

```
cron fires
  → reasoning job created
  → task 1: decompose
  → task 2: relate
```

---

## Task 1 — Decompose

**Input**: unprocessed session history + VAD time series, married on `turn_id`

**Goal**: extract every entity, theme, and atomic fact from the session. One actor per thing. No sentences. No relationships yet — that's task 2.

**Output**: observational actors written to the graph

**User turns are first class. System turns are context only.**

Entities are extracted from user signal. System turns are never mined for entities directly — they are used only to interpret the meaning of adjacent user turns.

```
system: "Is Mike your boyfriend?"
user:   "yes"
→ actor: Mike (entity) — anchored to user signal, system turn provided context

system: "Mike seems important to your work"
→ no actor — system assertion, no user signal to anchor it
```

```
skyra write_actor -type entity -layer observational \
  -content "late night" \
  -reasoning "skyra write_actor ... -m \"user turn: repeated across 6 sessions, user active after 11pm\""

skyra write_actor -type entity -layer observational \
  -content "Skyra project" \
  -reasoning "skyra write_actor ... -m \"user turn: primary topic of session, referenced 14 times\""
```

**Rules**:
- Entities are extracted from user turns only. System turns provide context to interpret user turns — they do not contribute entities on their own.
- One actor per entity. Do not merge entities — keep them atomic. When in doubt, create separate actors. Integrate will resolve duplicates via `alias_of` edges. False positives are better than merging distinct entities.
- `content` is the label for the thing. Not a sentence. Not a relationship.
- Every actor gets a `reasoning` field. Skyra explains why she created it, and whether a system turn was used as context.
- Confidence score derived from frequency + affect signal in the session.

---

## Task 2 — Relate

**Input**: actors produced by task 1 + existing graph (committed + observational)

**Goal**: reason over relationships between the new actors and the existing graph. Write edges.

**Output**: observational edges written to the graph

```
skyra write_edge \
  -from "entity:mike" -to "entity:skyra_project" \
  -type works_on -weight 0.9 \
  -reasoning "skyra write_edge ... -m \"14 references to skyra project across session, high arousal, positive valence\""

skyra write_edge \
  -from "entity:mike" -to "entity:late_night" \
  -type works_at -weight 0.7 \
  -reasoning "skyra write_edge ... -m \"session timestamps consistently 11pm-2am, VAD arousal elevated\""

skyra write_edge \
  -from "entity:skyra_project" -to "entity:late_night" \
  -type associated_with -weight 0.6 \
  -reasoning "skyra write_edge ... -m \"co-occurrence — skyra project work clusters with late night sessions\""
```

**Rules**:
- Edges are anchored to user signal. A system turn alone cannot produce an edge. System turns can provide context to interpret what the user meant, but the user turn must be the anchor.
- Check for existing actors before creating relationships to duplicates. Use `alias_of` if an existing actor represents the same entity.
- Skyra can add edges to committed actors freely.
- Every edge gets a `reasoning` field. If a system turn was used as context, note it.
- Weight derived from frequency, affect magnitude, and co-occurrence signal.
- Edge types are not exhaustive — new types can emerge.

---

## State Contract

Writes to the observational layer only. No user gate. No committed writes.

---

## Validation Criteria

- Every entity mentioned in user turns has a corresponding actor
- No actor is derived solely from a system turn
- Every actor has a `reasoning` field
- Every edge has a `reasoning` field
- No actor `content` is a sentence — atomic labels only
- No duplicate actors for the same entity

---

## Skill Contract

```
skill: reasoning
tasks:
  1. decompose  — session history + VAD → observational actors
  2. relate     — actors + existing graph → observational edges
boundary_rules:
  write_actor (non-skill types): allow_always (observational layer)
  write_actor (skill type):      deny
  write_edge:                   allow_always (observational layer + edges to committed actors)
  read_graph:                   allow_always
state_contract: working (observational only, no approval required)
severity_policy:
  duplicate entity detected: adjust locally — resolve to existing actor
  missing session data:      log and continue — partial pass is valid
replan_budget: 2
```

---

## Related

- `docs/arch/v1/memory/memory-structure.md` — actor + edge schema, two-tier graph, cron pass
- `docs/arch/v1/memory/observational-store.md` — four streams, VAD, session history
- `docs/arch/v1/skill/skill-lifecycle.md` — how skills crystallize from observational actors
- `docs/arch/v1/kernel/kernel.md` — cron service, job execution
