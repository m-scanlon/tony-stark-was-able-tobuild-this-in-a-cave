...
1. RAM pressure
   ...
   → evict observational actors (never committed)
...
trigger: system_health emits disk pressure event + shard available
  ...
  → verify integrity (actor count, edge count, checksum)
...
- **Committed layer is never evicted.** Only observational actors are candidates for dropping under RAM pressure.
