...
**There is no edit operation for skills.** Modification means a new skill with a new ID. The old skill continues to exist at its original hash. `provision_skill` of a new version is the only path (legacy term: `update_skill`) — it produces a new actor with a new ID, not a mutation of the old one.
...
- `read` — the skill can read from memory (observational + committed). It cannot write actors or edges. It cannot issue `write_actor` or `write_edge`. Kernel enforces at dispatch time.
...
```
commit {
  actor_ids:      []string        // actors being committed
  edge_ids:      []string        // edges being committed
  committed_at:  timestamp
  signed_by:     user_public_key
  signature:     bytes           // Ed25519 signature over (actor_ids + edge_ids + committed_at)
}
```
