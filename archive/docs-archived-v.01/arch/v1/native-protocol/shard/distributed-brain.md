...
The Brain Shard is currently the elected brain because it is the most capable available actor. That is not a permanent assignment. It is the current best option.
...
The system runs a primary-replica architecture. One actor is the current brain (Leader). All other capable actors are replicas (Followers), continuously synchronized and ready to be promoted.
...
If the Leader detects it is going down gracefully (resource exhaustion, planned shutdown), it broadcasts a distress signal to all known actors and can recommend a successor. This is faster than waiting for timeout but the system recovers either way.

### Capability-Aware Election

Standard Raft elects the actor with the most complete log. Skyra adds a capability dimension.

```
Election criteria:
  1. Log must be current (standard Raft requirement)
  2. Actor must meet minimum capability threshold (RAM, CPU floor)
  3. Among eligible candidates, most capable actor wins
```

A Voice Shard cannot be elected brain even if it has the freshest log. A actor below the capability floor is a Follower only.
...
- Do not assume there is only one control plane actor in the code
...
Who are the current healthy actors and their capabilities?
```
...
- **Raft vs simpler alternatives** — full Raft is complex. For a small personal cluster (3-5 actors), a simpler leader election mechanism may be sufficient. Evaluate against etcd/NATS before deciding to roll anything custom.
- **Replication protocol** — how commits stream from Leader to Followers. Frequency, consistency guarantees, conflict handling.
- **Capability floor definition** — minimum specs for a actor to be eligible as brain. Will vary as Skyra's services evolve.
- **Placement engine design** — algorithm that matches workloads to actors. How it weights RAM vs CPU vs GPU vs latency. How it handles colocation requirements.
- **Migration orchestration** — who coordinates a migration? What happens if it fails mid-transfer?
- **Split-brain handling** — if the network partitions and two actors both think they are Leader, state can diverge. Needs a defined resolution strategy.
- **Security** — actors accepting promotions and state from other actors is a significant attack surface. Trust model between actors needs definition.
