...
- `seed_refs[]`: list of `actor:<id>` or `edge:<id>`
- `context_units_left`: max number of context units remaining in budget
- `unit_mode`: `actor | actor_edge_pair`
...
- `actor_types_allow[]`: allowed actor types
...
- `time_window`: optional time bound for candidate edges/actors
- `dedupe_mode`: `actor_id | path`
