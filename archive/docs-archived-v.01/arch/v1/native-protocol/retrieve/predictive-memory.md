...
Skyra's memory is a **property graph where every actor and every edge carries a vector embedding**.

```
actor: entity, fact, skill, domain
edge: relationship between actors
...
The graph of committed actors and edges. Every actor here required a user-approved commit. This is the source of truth — high trust, auditable, every change reversible.
...
AND pattern.affect_magnitude > affect_threshold:
  → synthesize conclusion → propose actor for authoritative graph
```
...
long_term_memory {
  ...
  entities[]            // actor ids this memory involves
...
Entities are the named things in the user's life — people, places, tools, concepts — that persist across turns and sessions. They are actors in the graph. Without an entity layer, retrieval operates at domain level. With it, retrieval operates at entity level: "this is specifically about nginx_config, and here is everything Skyra knows about it."
...
- `docs/arch/v1/memory/memory-structure.md` — graph data structure, actor/edge schema
