# Attention and Inference

This document defines the relationship between continuous experience, attention, inference, act, and learning.

## Core Principle

The system is always experiencing.

Experience does not begin only when the user speaks.

Attention determines which part of current experience gets cognitive priority.

Inference operates on what attention selects.

Act is one possible outcome of inference, not its definition.

Learning is the later retention process that extracts lasting artifacts from experience.

## Experience

Experience is continuous.

The system may be:

- observing the user
- observing the environment
- observing itself
- undergoing internal runtime changes

All of this belongs to experience, whether or not there is any outward act.

## Attention

Attention allocates limited cognitive budget.

It determines which slice of current experience receives priority for inference.

Attention may be influenced by:

- user presence
- active episode state
- salience
- unresolved tension
- environmental change
- system goals

## Inference

Inference is attended cognition.

Inference does not require act.

Inference may:

- clarify
- interpret
- simulate
- compare
- decide
- update runtime artifacts

Act is only one possible result of inference.

## Act

Act is outward action or response.

It is not the same thing as inference.

The system may infer without acting.

The system may also act after inference when outward response is appropriate.

## Learning

Learning is the later process that turns experience into retained artifacts.

Not every attended experience is retained.

Learning remains selective.

## User Presence

If the user is present, user-facing experience should receive primary attention by default.

Other ongoing experience may continue, but with lower priority unless something stronger demands attention.

## User Absence

If the user is absent, the system may attend more freely to internal, environmental, or system-facing experience.

Inference may still occur without any outward act.

## Relationship to Recall

Attention shapes what is active in the current episode representation.

That representation helps drive background recall and explicit recall.

## Short Form

- experience is continuous
- attention is selective
- inference is attended cognition
- act is optional
- learning is delayed retention
